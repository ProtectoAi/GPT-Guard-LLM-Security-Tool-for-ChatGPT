        ...
        <section className={styles.container} data-is-scrollable aria-label={"chat history panel"}>
            ...
            <Stack aria-label="chat history panel content"
                ...
                <Stack className={styles.chatHistoryListContainer}>
                    {(appStateContext?.state.chatHistoryLoadingState === ChatHistoryLoadingState.Success && appStateContext?.state.isPostgresDBAvailable.postgresDB) && <ChatHistoryList/>}
                    {(appStateContext?.state.chatHistoryLoadingState === ChatHistoryLoadingState.Fail && appStateContext?.state.isPostgresDBAvailable) && <>
                        <Stack>
                            <Stack horizontalAlign='center' verticalAlign='center' style={{ width: "100%", marginTop: 10 }}>
                                <StackItem>
                                    <Text style={{ alignSelf: 'center', fontWeight: '400', fontSize: 16 }}>
                                        {appStateContext?.state.isPostgresDBAvailable?.status && <span>{appStateContext?.state.isPostgresDBAvailable?.status}</span>}
                                        {!appStateContext?.state.isPostgresDBAvailable?.status && <span>Error loading chat history</span>}
